IF EXISTS (select * from syscomments where id = object_id ('dbo.SP_GetCart'))
		...
		IF EXISTS (select * from syscomments where id = object_id ('dbo.SP_GetCartShippingPoint'))
		BEGIN
			DROP PROCEDURE dbo.SP_GetCartShippingPoint
		END
		GO
		-- =============================================
		-- Author:		Erich Keane
		...
		-- Description:	A stored procedure to get the current user's cart for the web project
		-- =============================================
		CREATE PROCEDURE SP_GetCartShippingPoint 
		( @CartId uniqueidentifier = null)
		as
		BEGIN
			-- SET NOCOUNT ON added to prevent extra result sets from
			...
			select 'Mount Vernon' as City, 'WA' as State,'98273' as Zip, 'USA' as Country
		END
